...
def NewInt():
    ...
    while OP2 == True:
        ...
        while True:#ip validation loop
            try:
                socket.inet_aton(NewMask) #checks if ip is valid eg will fail if ip is 192.168.1.999
                print('trueMask')
                OP2 = False
                break
    ...
    Session.sendline('ip add',NewIP,NewMask) # add the inputted ipaddress and subnet mask to the interface 
...
def NewProto():
    ...
    #OP 1 valadates ip for the first connection R1 > R2
    OP2 = True
    while OP2 == True:
        while True: #ip foramtting loop, checks the user input goes x.x.x.x192.168.2.1
            RNet = input('IP: ')
    ...
    Session.sendline('network',R1Net) # IP for the PC > R1 connection 
    Session.sendline('network',RNet) #IP for R1 > R2 connection
