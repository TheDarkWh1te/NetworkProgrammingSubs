...
#NOTE: logfile shows the comands to enter show run, test ways to remove this
def ShowRun():
    ...
    Session.sendline('show run') # send the show run comant to print the running config in the terminal 
    Session.logfile = Output #use logfile to copy and print wwhat comes through the terminal to the output var file
...
#NOTE: logfile shows the comands to enter show run, test ways to remove this
def ShowRun2():
    Output2 = open('ShowRunOut2.txt','w') #create and open a new file in which the running config will be saved 
    Session.sendline('terminal length 0') # set the terminal length to capture all of the running config in one print
    Session.sendline('show run') # send the show run comant to print the running config in the terminal 
    Session.logfile = Output2 #use logfile to copy and print wwhat comes through the terminal to the output var file
    Session.expect([pexpect.TIMEOUT,pexpect.EOF]) #
    #Output2.close() #close file
    print('Running config copied to file succsessfully') 

    Output = open('ShowRunOut2.txt','w') #create and open a new file in which the running config will be saved 
    Session.sendline('terminal length 0') # set the terminal length to capture all of the running config in one print
    Session.sendline('show run') # send the show run comant to print the running config in the terminal 
    Session.logfile = Output #use logfile to copy and print wwhat comes through the terminal to the output var file
    Session.expect([pexpect.TIMEOUT,pexpect.EOF]) #
